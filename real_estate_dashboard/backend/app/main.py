...
async def lifespan(app: FastAPI):
    ...
    if settings.ENABLE_INTEGRATIONS:
        ...
        try:
            from app.integrations.loader import initialize_integrations
            initialize_integrations()
            logger.info("✅ Third-party integrations initialized")
        except Exception as e:
            logger.warning(f"⚠️ Integration initialization failed: {e}")
    ...
    if settings.ENABLE_REDIS_CACHE:
        ...
        try:
            ...
            await init_cache_service(redis_client)
            logger.info("✅ Redis cache initialized and connected")
        except Exception as e:
            logger.warning(f"⚠️ Redis connection failed, using in-memory cache only: {e}")
            await init_cache_service(None)
    else:
        logger.info("Redis cache disabled, using in-memory cache only")
        await init_cache_service(None)
